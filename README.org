...
*** Data and views
    ...
    - int completes(): commences+((withdrawn+value) / rate) which
      should be constant
*** methods
    - MinimalAllowance(benefactor, rate, commences): should be created
      with ether value.  There is no other way to fund the contract.
    - afterCommencing payBenefactor(): transfer all currently-owed
      ether to benefactor.  If this payment reduces contract's value
      to zero, selfdestruct the contract.
    - onlyOwner burn(): call payBenefactor() then selfdestruct,
...
** Cashout version
   Owner can cashout the contract to the benefactor at any time.
...
*** methods
    - onlyOwner cashout(): transfer all remaining ether to benefactor
...
** controlled payout version
   Only owner can call payBenefactor()
